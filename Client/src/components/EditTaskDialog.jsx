...
export default function EditTaskDialog({ showEditTask, setShowEditTask, task, projectId }) {
    ...
    const handleAiSuggest = async () => {
        ...
        try {
            const response = await api.post("/ai/suggest", { title: formData.title });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await dispatch(updateTask(mapped)).unwrap();
            ...
            <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg shadow-lg w-full max-w-md p-6 text-zinc-900 dark:text-white">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <input type="date" value={formData.due_date} onChange={(e) => setFormData({ ...formData, due_date: e.target.value })} className="w-full rounded dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-zinc-900 dark:text-zinc-200 text-sm mt-1 focus:outline-none" />
                    </div>

                    {/* Action buttons */}
                    <div className="flex justify-end gap-2 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                        <button type="button" onClick={() => setShowEditTask(false)} className="px-4 py-2 border border-zinc-300 dark:border-zinc-700 rounded text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-150 dark:hover:bg-zinc-800 transition cursor-pointer" >
                            Cancel
                        </button>
                        <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-blue-600 text-white rounded text-sm hover:bg-blue-700 transition disabled:opacity-50 cursor-pointer" >
